...
@Injectable()
export class UpdatePoolsHoursService {
  private readonly BLOCK_OFFSET = 5;
  private readonly logger = new Logger(UpdatePoolsHoursService.name);
  ...
  @Cron('5 0-23 * * *') // Run 5 minutes after each hour (00:05, 01:05, etc.) until 23:05 UTC
  async updatePoolsHours() {
    try {
      this.logger.log('Starting hourly pools update...');
      ...
      for (const { network, pairs } of networkTokenPairs) {
        ...
        for (const pair of pairs) {
          try {
            ...
            const pools = await this.poolService.fetchPoolsHourlyUpdate(
              pair.token0.address,
              pair.token1.address,
              network,
      ...
      this.logger.log('Hourly pools update completed successfully');
    } catch (error) {
      this.logger.error(`Error in hourly pools update: ${error.message}`, error.stack);
    }
  }
}
